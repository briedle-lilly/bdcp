...
##################BEE SURE TO GO BACK AND DELETE THESE#####################################
  ...
  ##Drawing a sample of y values for the ith iteration.  We are forcing there to be at least 5 zeroes and 5 ones in the original sample
    ...
    for(b in 1:B){
      boot.disc.comparisons[b,] <- log.reg.draw.boot.and.calc.disc.comparisons.fxn(
...
log.reg.fxn <- function(iter, n, B, true.beta0, true.beta1, true.beta2, true.beta3, true.beta4){
  ...
  list.log.reg.data <- list("Wald.full.null.pval" = round(log.reg.data[,1],5) ,
             "Wald.partial.null.pval" = round(log.reg.data[,2],5) , 
              "score.full.null.pval" = round(log.reg.data[,3],5),
              "score.partial.null.pval" = round(log.reg.data[,4],5),
              "LRT.full.null.pval" = round(log.reg.data[,5],5),
              "LRT.partial.null.pval" = round(log.reg.data[,6],5) ,
          "Wald.full.null.disc.comparison.prob" = round(log.reg.data[,7],5) ,
         "Wald.partial.null.disc.comparison" = round(log.reg.data[,8],5) ,
         "score.full.null.disc.comparison.probs" = round(log.reg.data[,9],5),
         "score.partial.null.disc.comparison"= round(log.reg.data[,10],5),
         "KL.full.null.disc.comparison" = round(log.reg.data[,11],5),
         "KL.partial.null.overall.disc.comparison" = round(log.reg.data[,12],5),
         "KL.partial.null.para.int.overall.disc.comparison"= round(log.reg.data[,13],5))
  
  return(list.log.reg.data)
